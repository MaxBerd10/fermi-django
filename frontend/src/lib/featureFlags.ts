/**
 * Integrations that depend on secrets/infra — off by default so the site
 * never renders a dead button or silently 404s against a proxy route that
 * isn't there. Flip one on by setting the matching VITE_FEATURE_* env var to
 * "true" (telegram needs no secret at all — see api/telegram.ts — the other
 * two need real credentials first).
 *
